...
@app.get("/listings/{listing_id}/ohlcv", tags=["listings"])
async def get_ohlcv_data(
    listing_id: UUID,
    asset_name: Optional[str] = Query(None, description="Filter by specific asset name"),
    interval: str = Query('1h', description="Candle interval (1m, 5m, 15m, 1h, 4h, 1d, 1w, 1M, 1Y, ALL)"),
    range: str = Query('1M', description="Time range (1D, 1W, 1M, 3M, 1Y, ALL)"),
    manager: ListingManager = Depends(get_listing_manager)
) -> List[dict]:
    """Get OHLCV (Open, High, Low, Close, Volume) candle data for a listing's sales history."""
    logger.info(f"Getting OHLCV data for listing {listing_id}")
    ...
    try:
        ...
        if interval_minutes is None and interval != 'ALL':
        ...
        # Get the interval text for date_trunc
        interval_text = {
            1: 'minute',
            5: 'hour',     # Will need post-processing
            15: 'hour',    # Will need post-processing
            60: 'hour',
            240: 'day',    # Will need post-processing
            1440: 'day',
            10080: 'week',
            43200: 'month',
            525600: 'year',
            None: 'month'  # For ALL interval, group by month
        }[interval_minutes]
            
        async with manager.pool.acquire() as conn:
            # Build query conditions and parameters
            conditions = ["listing_id = $2"]  # Note: $1 is reserved for interval_text
            params = [interval_text, listing_id]
            param_idx = 3
            
            if asset_name:
                conditions.append(f"asset_name = ${param_idx}")
                params.append(asset_name)
                param_idx += 1
                
            if start_time:
                conditions.append(f"sale_time >= ${param_idx}")
                params.append(start_time)
                param_idx += 1
            
            # Build the OHLCV query using window functions
            query = f"""
                WITH time_buckets AS (
                    SELECT 
                        date_trunc($1, sale_time) as bucket_start,
                        price_evr,
                        amount,
                        price_evr * amount as volume_evr
                    FROM sale_history
                ...
                candles AS (
                    SELECT 
                        ...
                        FIRST_VALUE(price_evr) OVER w as open,
                        MAX(price_evr) OVER w as high,
                        MIN(price_evr) OVER w as low,
                        LAST_VALUE(price_evr) OVER w as close,
                        SUM(amount) OVER w as volume,
                        SUM(volume_evr) OVER w as volume_evr,
            ...
            results = []
            for row in rows:
                # Post-process time buckets for non-standard intervals
                bucket_time = row['time']
                if interval_minutes in [5, 15]:
                    # Adjust to nearest 5 or 15 minute mark
                    ...
                    # Adjust to nearest 4 hour mark
                    hour = (bucket_time.hour // 4) * 4
                    bucket_time = bucket_time.replace(hour=hour)
