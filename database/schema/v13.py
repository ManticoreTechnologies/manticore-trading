"""Schema v13 - Add deposit address to listing balances for direct balance tracking."""
...
schema = {
    ...
    'tables': [
        ...
        },
        {
            ...
            'primary_key': ['deposit_address', 'asset_name'],  # Changed to use deposit_address as primary key
            'foreign_keys': [
                {'columns': ['listing_id'], 'references': 'listings(id)'},
                {'columns': ['deposit_address'], 'references': 'listing_addresses(deposit_address)'}
            ],
            'indexes': [
                {'name': 'listing_balances_by_listing', 'columns': ['listing_id', 'asset_name']},
    ...
    'migrations': [
        ...
        # Create listing_addresses table
        '''
        ...
        # Create listing_balances table with deposit_address
        '''
        CREATE TABLE listing_balances (
            listing_id UUID NOT NULL REFERENCES listings(id),
            deposit_address STRING NOT NULL REFERENCES listing_addresses(deposit_address),
            asset_name STRING NOT NULL,
            ...
            updated_at TIMESTAMP NOT NULL DEFAULT now(),
            PRIMARY KEY (deposit_address, asset_name)
        );
        CREATE INDEX listing_balances_by_listing ON listing_balances(listing_id, asset_name);
