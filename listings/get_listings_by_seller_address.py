...
async def get_listings_by_seller_address(seller_address: str, offset: int = 0, limit: int = 10, pool=None) -> Dict[str, Any]:
    """Get listings by seller address with all related data and pagination metadata.
    
    Args:
        seller_address: The address of the seller to filter listings
    ...
    async with pool.acquire() as conn:
        ...
        # Get base listing with pagination and filter by seller address
        listings = await conn.fetch(
            'SELECT * FROM listings WHERE seller_address = $1 OFFSET $2 LIMIT $3',
            seller_address, offset, limit
        )
        
        # Get total count of listings for the seller
        total_count = await conn.fetchval(
            'SELECT COUNT(*) FROM listings WHERE seller_address = $1',
            seller_address
        )
        
        if not listings:
            raise LookupError("No listings found for the given seller address")
