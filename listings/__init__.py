        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                query = '''
                    SELECT DISTINCT l.*
                    FROM listings l
                    LEFT JOIN listing_prices p ON l.id = p.listing_id
                '''
                ...
                if search_term:
                    conditions.append(
                        "(l.name ILIKE $1 OR l.description ILIKE $1 OR l.tags ILIKE $1)"
                    )
                    params.append(f"%{search_term}%")
                
                if seller_address:
                    conditions.append(f"l.seller_address = ${len(params) + 1}")
                    params.append(seller_address)
                
                if asset_name:
                    conditions.append(f"p.asset_name = ${len(params) + 1}")
                    params.append(asset_name)
                
                if min_price_evr is not None:
                    conditions.append(f"p.price_evr >= ${len(params) + 1}")
                ...
                # Add pagination
                query += " ORDER BY l.created_at DESC LIMIT $" + str(len(params) + 1)
                params.append(limit)
