...
async def get_listings_by_asset_name(asset_name: str, offset: int = 0, limit: int = 10, pool=None) -> Dict[str, Any]:
    """Get listings by asset name with all related data and pagination metadata.
    
    Args:
        asset_name: The name of the asset to filter listings
    ...
    async with pool.acquire() as conn:
        ...
        # Get base listing with pagination and filter by asset name
        listings = await conn.fetch(
            'SELECT * FROM listings WHERE id IN (SELECT listing_id FROM listing_prices WHERE asset_name = $1) OFFSET $2 LIMIT $3',
            asset_name, offset, limit
        )
        
        # Get total count of listings for the asset
        total_count = await conn.fetchval(
            'SELECT COUNT(*) FROM listings WHERE id IN (SELECT listing_id FROM listing_prices WHERE asset_name = $1)',
            asset_name
        )
        
        if not listings:
            raise LookupError("No listings found for the given asset name")
        ...
        prices = await conn.fetch(
            'SELECT * FROM listing_prices WHERE asset_name = $1',
            asset_name
        )
