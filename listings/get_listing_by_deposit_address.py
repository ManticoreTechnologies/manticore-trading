...
async def get_listing_by_deposit_address(deposit_address: str, pool=None) -> Dict[str, Any]:
    """Get a listing by deposit address with all related data.
    ...
    async with pool.acquire() as conn:
        ...
        listing = await conn.fetchrow(
            'SELECT * FROM listings WHERE deposit_address = $1',
            deposit_address
        )
        
        if not listing:
            raise LookupError(f"Listing {deposit_address} not found")
