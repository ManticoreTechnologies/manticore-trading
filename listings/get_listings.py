...
async def get_listings(offset: int = 0, limit: int = 10, pool=None) -> Dict[str, Any]:
    """Get all listings with all related data and pagination metadata.
    ...
    async with pool.acquire() as conn:
        ...
        # Get base listing with pagination
        listings = await conn.fetch(
            'SELECT * FROM listings OFFSET $1 LIMIT $2',
            offset, limit
        )
        
        # Get total count of listings
        total_count = await conn.fetchval('SELECT COUNT(*) FROM listings')
        
        if not listings:
            raise LookupError("No listings found")
